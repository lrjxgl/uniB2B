	...
	export default {
		...
		methods: {
			...
			goGuest: function(shopid) {
				uni.navigateTo({
					url: "../b2b_guest/user?shopid=" + shopid
				})
			},
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/b2b_order/show?orderid=" + this.orderid,
			...
			cancel: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/b2b_order/cancel?orderid=" + this.orderid,
					success: function(rs) {
						that.getPage();
					}
				})
			...
			receive: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/b2b_order/receive?orderid=" + this.orderid,
					success: function(rs) {
						that.getPage();
					}
				})
			...
			goRaty: function() {
				uni.navigateTo({
					url: "../b2b_order/raty?orderid=" + this.orderid
				})
			},
			pay: function() {
				uni.navigateTo({
					url: "../b2b_order/pay?orderid=" + this.orderid
				})
			}
		}
	}
